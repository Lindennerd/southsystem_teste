function Users() {
    ...
    if (self.users.length > 0) {
      self.users.forEach(user => {
        ...
        if (user.favoriteBooks) {
          ...
          item.classList.add("collection-item");
          item.textContent =
            "Nenhum livro adicionado a lista de favoritos desse usuário";
  ...
  function editUser(e) {
    ...
    const editingUser = self.users.find(user => {
  ...
  async function saveUser() {
    if (false) { //!auth().user
      ...
      if (
        self.$newUserForm.userPassword.value ===
        self.$newUserForm.userPasswordConfirmation.value
      ) {
        if (formIsValid(self.$newUserForm)) {
          ...
          if (response.status === 200) {
            ...
            M.toast({
              html: "Ocorreu um erro ao salvar o Usuário",
              classes: "red",
              displayLength: 4000
            });
          }
